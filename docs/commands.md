...
生成**可视化分析报告**：在输出目录产出 `report.html`（自包含的单文件 HTML，内联 CSS、无外部依赖，浏览器直接打开即可看）。报告包含：

- 项目摘要：目标类型、编译器、并行度、执行器、源文件数、符号总数
- 目标符号表：逐源文件列出目标文件的符号（`nm`），自动覆盖配置里的全部源文件
- 编译命令：`compile_commands.json` 里的原样命令
- 构建产物分析：`log/` 下的符号表、动态依赖、readelf 分析（构建后才有）
- 日志产物：`log/` 目录的文件与大小清单
