...
> 边界已定（2026-09-16）：**antel 决定编什么，make 负责并行编完**。不产出面向用户的 Makefile；内部规则文件只是调用 make 的手段（输出目录内、每次重建、标为生成物）。
...
| 交付物 | 新增 `antelope/makefile.py`：把 `BuildPlan` 渲染成**内部规则文件**（逐单元显式规则 + 链接目标 + `-include` + 生成标记），写入 `<输出目录>/antel.mk`；新增 make 执行器，调用形态为 `make -f <规则> -j$jobs <过期目标>` |
