...
>
> 进度：**P1-2 ✅ 已完成（2026-09-17）**；P1-1（sync-baseline）、P1-3（`auto` 默认与回退）、P1-4（跨实现一致性测试）待做。
...
| 2026-09-17 | M1 / P1-2 | 规则文件渲染 + make 执行器 + `backend` 字段（显式 antel/make）；先删目标、`$` 翻倍、`.DELETE_ON_ERROR`；测试 9 → 14 条；150 TU 两种执行器实测持平（收益不在速度，在 jobserver 与可复现） | 本批提交 |
